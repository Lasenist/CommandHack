...
const float scaleInLinearGamma = 1.0;
const float shadowMask = 1.0;
const float brightboost = 1.0;

const vec2 video_size = vec2(0.0, 0.0);
const vec2 texture_size = vec2(0.0, 0.0);
const vec2 output_size = vec2(0.0, 0.0);
...
vec3 Fetch(vec2 pos,vec2 off)
{
	...
	return ToLinear(brightboost * texture(tex0, pos.xy).rgb);
}
